use super::{HelmholtzEnergyWrapper, ResidualHelmholtzEnergy};
use feos_core::{
    HelmholtzEnergyDerivatives, PhaseEquilibrium, ReferenceSystem, State,
};
...
pub type StateAD<'a, E, D, const N: usize> =
    State<HelmholtzEnergyWrapper<'a, E, D, N>, D, Const<N>, ()>;

pub type PhaseEquilibriumAD<'a, E, D, const N: usize> =
    PhaseEquilibrium<HelmholtzEnergyWrapper<'a, E, D, N>, D, Const<N>, (), 2>;
    ...
    HelmholtzEnergyDerivatives<D> for HelmholtzEnergyWrapper<'a, E, D, N>
{
    ...
    type Real = HelmholtzEnergyWrapper<'a, E, f64, N>;
    ...
    fn _residual_helmholtz_energy(state: &StateAD<'a, E, D, N>) -> D {
        E::residual_molar_helmholtz_energy(
            state.eos.parameters,
            state.temperature.into_reduced(),
            state.density.into_reduced().recip(),
            &state.molefracs,
        )
    }

    fn _residual_entropy(state: &StateAD<'a, E, D, N>) -> D {
        let params = E::params_from_inner(state.eos.parameters);
        let molar_volume = Dual::from_re(state.volume.into_reduced());
        let molefracs = state.molefracs.map(Dual::from_re);
    ...
    fn _residual_pressure(state: &StateAD<'a, E, D, N>) -> D {
        let params = E::params_from_inner(state.eos.parameters);
        let temperature = Dual::from_re(state.temperature.into_reduced());
        let molefracs = state.molefracs.map(Dual::from_re);
        let (_, da_dv) = first_derivative(
            |molar_volume| {
                E::residual_molar_helmholtz_energy(&params, temperature, molar_volume, &molefracs)
            },
            state.volume.into_reduced(),
        );
        -da_dv
    }

    fn _residual_chemical_potential(state: &StateAD<'a, E, D, N>) -> SVector<D, N> {
        let params = E::params_from_inner(state.eos.parameters);
        let temperature = DualVec::from_re(state.temperature.into_reduced());
        let molar_volume = DualVec::from_re(state.volume.into_reduced());
        let (_, da_dx) = gradient(
            |molefracs| {
                E::residual_molar_helmholtz_energy(&params, temperature, molar_volume, &molefracs)
            },
            state.molefracs,
    ...
    fn _dp_res_dv(state: &StateAD<'a, E, D, N>) -> D {
        let params = E::params_from_inner(state.eos.parameters);
        let temperature = Dual2::from_re(state.temperature.into_reduced());
        let molefracs = state.molefracs.map(Dual2::from_re);
        let (_, _, d2a_dv2) = second_derivative(
            |molar_volume| {
                E::residual_molar_helmholtz_energy(&params, temperature, molar_volume, &molefracs)
            },
            state.volume.into_reduced(),
    ...
    fn _dp_res_dt(state: &StateAD<'a, E, D, N>) -> D {
        let params = E::params_from_inner(state.eos.parameters);
        let molefracs = state.molefracs.map(HyperDual::from_re);
        let (_, _, _, d2a_dtdv) = second_partial_derivative(
    ...
    fn _d2p_res_dv2(state: &StateAD<'a, E, D, N>) -> D {
        let params = E::params_from_inner(state.eos.parameters);
        let temperature = Dual3::from_re(state.temperature.into_reduced());
        let molefracs = state.molefracs.map(Dual3::from_re);
        let (_, _, _, d3a_dv3) = third_derivative(
            |molar_volume| {
                E::residual_molar_helmholtz_energy(&params, temperature, molar_volume, &molefracs)
            },
            state.volume.into_reduced(),
    ...
    fn _ds_res_dt(state: &StateAD<'a, E, D, N>) -> D {
        let params = E::params_from_inner(state.eos.parameters);
        let molar_volume = Dual2::from_re(state.volume.into_reduced());
        let molefracs = state.molefracs.map(Dual2::from_re);
        let (_, _, d2a_dt2) = second_derivative(
            |temperature| {
                E::residual_molar_helmholtz_energy(&params, temperature, molar_volume, &molefracs)
            },
            state.temperature.into_reduced(),
    ...
    fn _d2s_res_dt2(state: &StateAD<'a, E, D, N>) -> D {
        let params = E::params_from_inner(state.eos.parameters);
        let molar_volume = Dual3::from_re(state.volume.into_reduced());
        let molefracs = state.molefracs.map(Dual3::from_re);
        let (_, _, _, d3a_dt3) = third_derivative(
            |temperature| {
                E::residual_molar_helmholtz_energy(&params, temperature, molar_volume, &molefracs)
            },
            state.temperature.into_reduced(),
    ...
    fn _dmu_res_dn(state: &StateAD<'a, E, D, N>) -> OMatrix<D, Const<N>, Const<N>> {
        todo!()
    }

    fn residual_helmholtz_energy_density<D2: DualNum<f64, Inner = D> + Copy>(
        &self,
        temperature: D2,
        partial_density: &SVector<D2, N>,
    ) -> D2 {
        let parameters = E::params_from_inner(self.parameters);
        E::residual_helmholtz_energy_density(&parameters, temperature, partial_density)
    }
    ...
    fn reduced_residual_molar_helmholtz_energy<
        D3: DualNum<f64, Inner = D2> + Copy,
        D2: DualNum<f64, Inner = D> + Copy,
    >(
        &self,
        temperature: D3,
        molar_volume: D3,
        molefracs: &nalgebra::OVector<D3, Self::Components>,
    ) -> D3 {
        let parameters = E::params_from_inner(&E::params_from_inner(self.parameters));
        E::residual_molar_helmholtz_energy(&parameters, temperature, molar_volume, molefracs)
    }

    fn dmu_dni<D2: DualNum<f64, Inner = D> + Copy>(
        &self,
        temperature: D2,
        molar_volume: D2,
        molefracs: &SVector<D2, N>,
    ) -> OMatrix<D2, Const<N>, Const<N>> {
        let t = Dual2Vec::from_re(temperature);
        let v = Dual2Vec::from_re(molar_volume);
        let (_, _, m) = hessian(
            |x| self.reduced_residual_molar_helmholtz_energy(t, v, &x),
            *molefracs,
        );
        m
    }

    // fn eigen<D2: DualNum<f64> + Copy>(
    //     matrix: OMatrix<D2, Const<N>, Const<N>>,
    // ) -> (D2, SVector<D2, N>) {
    //     <Const<N> as Eigen<N>>::eigen(matrix)
    // }
    ...
    // ) -> Dual<D, f64> {
    //     // calculate M
    //     let molefracs = molefracs.map(Dual::from_inner);
    //     let parameters = E::params_from_inner(self.parameters);
    ...
    //     let (l, _) = <Const<N> as Eigen<N>>::eigen(m);
    ...
    //     density: DualVec<D, f64, Const<2>>,
    //     molefracs: &SVector<D, N>,
    // ) -> SVector<DualVec<D, f64, Const<2>>, 2> {
    //     // calculate M
    //     let molefracs = molefracs.map(DualVec::from_inner);
    //     let parameters = E::params_from_inner(self.parameters);
    //     let molar_volume = density.recip();
    //     let sqrt_z = molefracs.map(|z| z.sqrt());
    //     let z_mix = sqrt_z * sqrt_z.transpose();
    ...
    //         |rho| self.residual_helmholtz_energy_density(t, &rho),
    //         *partial_density,
    //     );
    ...
    //         |x, v| self._residual_molar_helmholtz_energy(t, v[0], &x),
    //         *molefracs,
...
mod test {
    ...
    use crate::{EquationOfStateAD, ParametersAD};
    ...
    fn test_critical_point() -> FeosResult<()> {
        ...
        let mut params_dual = pcsaft.map(Dual64::from);
        params_dual[0] = params_dual[0].derivative();
        let pcsaft = pcsaft.derivatives(&params_dual);
        let cp = State::critical_point(&eos, None, None, Default::default())?;
        let state = StateAD::critical_point(&pcsaft, None, None, Default::default())?;
        ...
        let h = 1e-8;
        params[0] += h;
        let eos = PcSaftPure(params);
        let eos = eos.wrap();
        let cp_h = StateAD::critical_point(&eos, None, None, Default::default())?;
        let dt = (cp_h.temperature - cp.temperature).to_reduced() / h;
        let drho = (cp_h.density - cp.density).to_reduced() / h;

        let dt_ad = state.temperature.into_reduced().eps;
        let drho_ad = state.density.into_reduced().recip().eps;
        println!("{dt_ad:.5e} {drho_ad:.5e}",);
        println!("{dt:.5e} {drho:.5e}");
        assert_relative_eq!(dt_ad, dt, max_relative = 1e-6);
        assert_relative_eq!(drho_ad, drho, max_relative = 1e-6);
        Ok(())
    }
    ...
    fn test_state_tp_derivative() -> FeosResult<()> {
        ...
        let eos_ad = pcsaft.derivatives(&params);
        ...
        let density = state.density.into_reduced();
