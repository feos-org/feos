use super::parameters::{PcSaftAssociationRecord, PcSaftParameters};
...
impl PcSaft {
    ...
    pub fn with_options(parameters: PcSaftParameters, options: PcSaftOptions) -> Self {
        ...
        let hard_chain = parameters
            .pure_records
            .iter()
            .any(|pr| (pr.model_record.m - 1.0).abs() > 1e-15)
            .then_some(HardChain::new(&parameters));
...
impl HardSphereProperties for PcSaft {
...
impl AssociationStrength for PcSaft {
...
impl Residual for PcSaft {
    fn compute_max_density(&self, moles: &Array1<f64>) -> f64 {
        self.options.max_eta * moles.sum()
            / (FRAC_PI_6 * &self.m * self.sigma.mapv(|v| v.powi(3)) * moles).sum()
    }
    ...
    fn residual_helmholtz_energy_contributions<D: DualNum<f64> + Copy>(
        ...
        let (a_hs, [_, _, zeta2, zeta3], d) =
            HardSphere.helmholtz_energy_and_properties(self, state);

        v.push(("Hard Sphere".into(), a_hs));
        if let Some(hc) = self.hard_chain.as_ref() {
            v.push((
                "Hard Chain".into(),
                hc.helmholtz_energy(state, &d, zeta2, zeta3),
            ))
        }
        v.push((
            "Dispersion".into(),
        ...
        if let Some(dipole) = self.polar.helmholtz_energy_dd(state, zeta3) {
            v.push(("Dipole".into(), dipole))
        }
        if let Some(quadrupole) = self.polar.helmholtz_energy_qq(state, zeta3) {
            v.push(("Quadrupole".into(), quadrupole))
        }
        if let Some(dipole_quadrupole) =
            self.polar
                .helmholtz_energy_dq(self.options.dq_variant, state, zeta3)
        {
            v.push(("DipoleQuadrupole".into(), dipole_quadrupole))
        }
        if let Some(association) = self.association.helmholtz_energy(self, state, &d) {
            v.push(("Association".into(), association))
        }
        v
    }
}
...
impl Molarweight for PcSaft {
    fn molar_weight(&self) -> MolarWeight<Array1<f64>> {
        self.parameters.molar_weight() * GRAM / MOL
    }
}
...
mod tests {
    ...
    fn hard_sphere() {
        ...
        let eos = PcSaft::new(propane_parameters());
        let a_rust = HardSphere.helmholtz_energy(&eos, &s);
        assert_relative_eq!(a_rust, 0.410610492598808, epsilon = 1e-10);
    }

    #[test]
    fn hard_sphere_mix() {
        let propane = PcSaft::new(propane_parameters());
        let butane = PcSaft::new(butane_parameters());
        let mix = PcSaft::new(propane_butane_parameters());
        let t = 250.0;
        let v = 2.5e28;
        let n = 1.0;
        let s = StateHD::new(t, v, arr1(&[n]));
        let a1 = HardSphere.helmholtz_energy(&propane, &s);
        let a2 = HardSphere.helmholtz_energy(&butane, &s);
    ...
    fn association() {
        ...
        let assoc = Association::new(&parameters, 50, 1e-10);
        ...
        let a_rust = assoc.helmholtz_energy(&eos, &s, &d).unwrap() / n;
    ...
    fn cross_association() {
        let parameters = water_parameters();
        let assoc = Association::new_cross_association(&parameters, 50, 1e-10);
        let eos = PcSaft::new(parameters);
        let t = 350.0;
        let v = 41.248289328513216;
        let n = 1.23;
