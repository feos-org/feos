...
use crate::uvcs::{corresponding_states::CorrespondingParameters, parameters::*};
...
pub fn attractive_perturbation_helmholtz_energy_density<D: DualNum<f64> + Copy>(
    ...
    let u_fraction_wca = u_fraction_wca(rep_x, density * x.dot(&p.sigma.map(|s| s.powi(3))));
...
fn residual_virial_coefficient<D: DualNum<f64> + Copy>(
    ...
    for i in 0..p.ncomponents {
        ...
        for j in 0..p.ncomponents {
            ...
            let q_ij = dimensionless_diameter_q_wca(t_ij, rep_ij, att_ij);
...
pub(super) fn one_fluid_properties<D: DualNum<f64> + Copy>(
    ...
    let sigma_x = x.dot(&p.sigma.map(|v| v.powi(3))).powf(1.0 / 3.0);
...
fn delta_b2<D: DualNum<f64> + Copy>(reduced_temperature: D, rep: D, att: D, q: D) -> D {
    let rm = (rep / att).powd((rep - att).inv()); // Check mixing rule!!
    let rc = D::one() * 5.0;
...
fn y_eff<D: DualNum<f64> + Copy>(reduced_temperature: D, rep: D, att: D) -> D {
    // optimize: move this part to parameter initialization
    let rc = D::one() * 5.0;
    let rs = (rep / att).powd((rep - att).inv());
    let c0 = D::one()
        - (mean_field_constant(rep, att, rs) - mean_field_constant(rep, att, rc)) * 3.0
            / (rc.powi(3) - rs.powi(3));
    let ir = rep.inv();
    let c1 = ir * C2[0][1] + C2[0][0];
    let c2 = ir * C2[1][1] + C2[1][0];
    let c3 = ir * C2[2][1] + C2[2][0];
...
mod test {
    use super::*;
    use crate::uvcs::parameters::utils::{methane_parameters, test_parameters_mixture};
    ...
    fn test_attractive_perturbation() {
        ...
        let (rep_x, att_x, sigma_x, weighted_sigma3_ij, epsilon_k_x, d_x) =
            one_fluid_properties(&cp, &state.molefracs, state.temperature);
        dbg!(epsilon_k_x);
        let t_x = state.temperature / epsilon_k_x;
        ...
        let b2bar = residual_virial_coefficient(&cp, x, state.temperature) / p.sigma[0].powi(3);
        dbg!(b2bar);
        ...
        dbg!(a_test);
        dbg!(state.partial_density.sum());
        let a = attractive_perturbation_helmholtz_energy_density(&cp, &state) / moles[0];
        dbg!(a);
    ...
    fn test_attractive_perturbation_wca_mixture() {
        ...
        let delta_b2 = residual_virial_coefficient(&cp, &state.molefracs, state.temperature)
            / p.sigma[0].powi(3);
        ...
        let a =
            attractive_perturbation_helmholtz_energy_density(&cp, &state) / (moles[0] + moles[1]);
    ...
    fn test_attractive_perturbation_wca_mixture_different_sigma() {
        ...
        let state = StateHD::new(reduced_temperature, volume, &moles);
        let cp = CorrespondingParameters::new(&p, state.temperature);
        let (rep_x, att_x, sigma_x, weighted_sigma3_ij, epsilon_k_x, d_x) =
            one_fluid_properties(&cp, &state.molefracs, state.temperature);
        // u-fraction
