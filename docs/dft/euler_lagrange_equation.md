...
which shows that, by construction, the density of every segment on a molecule is identical in a bulk system. The index $\gamma$ refers to all segments on molecule $i$. The expressions can be combined in a similar way as for the molecular DFT:
