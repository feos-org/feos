...
$$
\mathcal{F}_\alpha\left(r;\left[\rho(r)\right]\right)=-\int\sum_\beta\frac{\delta\mathcal{F}_\alpha\left(r;\left[\rho(r)\right]\right)}{\delta\rho_\beta(r')}\Delta\rho_\beta(r')\mathrm{d}r'
$$ (eqn:newton)
...
The functional derivative can be simplified using $\hat F_{\rho_\alpha\rho_\beta}^\mathrm{res}(r,r')=\frac{\delta \hat F_{\rho_\alpha}^\mathrm{res}(r)}{\delta\rho_\beta(r')}=\frac{\delta^2\hat F^\mathrm{res}}{\delta\rho_\alpha(r)\delta\rho_\beta(r')}$
...
Here $\hat f^\mathrm{res}_{\alpha\beta}=\frac{\partial^2\hat f^\mathrm{res}}{\partial n_\alpha\partial n_\beta}$ is the second partial derivative of the reduced Helmholtz energy density with respect to the weighted densities $n_\alpha$ and $n_\beta$. The definition of the weighted densities $n_\alpha(r)=\sum_i\int\rho_i(r')\omega_\alpha^i(r-r')\mathrm{d}r'$ is used to simplify the expression further.
...
With the weighted-density variation $\Delta n_\beta(r'')=\sum_j\int\Delta\rho_j(r')\omega_\beta^j(r''-r')\mathrm{d}r'$, $\Delta\hat F_{\rho_\alpha}^\mathrm{res}(r)$ can be rewritten as

$$
\begin{aligned}
\Delta\hat F_{\rho_\alpha}^\mathrm{res}(r)&=\int\sum_{\alpha,\beta}\hat f^\mathrm{res}_{\alpha\beta}(r'')\omega_\alpha^i(r''-r)\Delta n_\beta(r'')\mathrm{d}r''\\
&=\int\sum_\alpha\sum_\beta \hat f^\mathrm{res}_{\alpha\beta}(r'')\Delta n_\beta(r'')\omega_\alpha^i(r''-r)\mathrm{d}r''\\
&=\int\sum_\alpha \Delta\hat f^\mathrm{res}_\alpha(r'')\omega_\alpha^i(r''-r)\mathrm{d}r''
\end{aligned}
$$ (eqn:newton_F)
...
&\qquad\times e^{\frac{\beta}{m_{\alpha'}}\left(\hat F_{\rho_{\alpha'}}^\mathrm{b,res}-\hat F_{\rho_{\alpha'}}^\mathrm{res}(r')-V_{\alpha'}^\mathrm{ext}(r')\right)}\left(\prod_{\alpha''\neq\alpha}I_{\alpha'\alpha''}(r')\right)\omega_\mathrm{chain}^{\alpha\alpha'}(r-r')\mathrm{d}r'
\end{aligned}
$$ (eqn:newton_I)
