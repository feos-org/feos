...
$$ n*\alpha(\mathbf{r})=\sum_in*\alpha^i(\mathbf{r})=\sum*i\int\rho_i(\mathbf{r}- \mathbf{r}')\omega*\alpha^i(\mathbf{r}')\mathrm{d}\mathbf{r}'.$$
...
$$ F[\mathbf{\rho}(\mathbf{r})]=\int\left(f(\mathbf{\rho})+\sum*{ij}\frac{c*{ij}(\mathbf{\rho})}{2}\nabla\rho_i\cdot\nabla\rho_j\right)\mathrm{d}\mathbf{r}$$

with the density dependent influence parameter

$$ \beta c*{ij}(\mathbf{\rho})=-\sum*{\alpha\beta}\frac{\partial^2\Phi}{\partial n*\alpha\partial n*\beta}\left(\omega*\alpha^{i2}\omega*\beta^{j0}+ \omega*\alpha^{i0}\omega*\beta^{j2}\right).$$
...
$$ \gamma=\frac{F-\mu N+pV}{A}=\int\_{\rho^\mathrm{V}}^{\rho^\mathrm{L}} \sqrt{2c \left(f(\rho)-\rho\mu+p\right) } d\rho $$
...
$$ z(\rho)=\int\_{\rho^\mathrm{V}}^{\rho} \sqrt{\frac{c/2}{ f(\rho)-\rho\mu+p} } d\rho $$
