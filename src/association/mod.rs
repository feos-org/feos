...
/// Trait used to interface the SAFT association contribution with arbitrary models.
pub trait AssociationProperties {
    fn association_parameters(&self) -> &AssociationParameters;
    fn hs_diameter<D: DualNum<f64>>(&self, temperature: D) -> Array1<D>;
    fn weighted_densities<D: DualNum<f64>>(&self, state: &StateHD<D>) -> [D; 2];
}
...
impl<P: AssociationProperties> Association<P> {
    pub fn new(parameters: &Rc<P>, max_iter: usize, tol: f64) -> Self {
        Self {
            parameters: parameters.clone(),
            association_parameters: parameters.association_parameters().clone(),
            max_iter,
            tol,
    ...
    pub fn new_cross_association(parameters: &Rc<P>, max_iter: usize, tol: f64) -> Self {
...
impl<D: DualNum<f64> + ScalarOperand, P: AssociationProperties> HelmholtzEnergyDual<D>
    for Association<P>
{
    fn helmholtz_energy(&self, state: &StateHD<D>) -> D {
        ...
        // auxiliary variables
        let [n2, n3] = p.weighted_densities(state);
...
impl<P: AssociationProperties> Association<P> {
    ...
    pub fn helmholtz_energy_density_cross_association<
        S: Data<Elem = D>,
