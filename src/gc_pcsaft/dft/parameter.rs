    ...
    fn from_segments<C: Into<ChemicalRecord>>(
        ...
        for (i, chemical_record) in chemical_records.iter().enumerate() {
            ...
            for id in &chemical_record.segments {
                ...
                association_records.push(segment.model_record.association_record.clone());
...
impl AssociationProperties for GcPcSaftFunctionalParameters {
    ...
    fn weighted_densities<D: num_dual::DualNum<f64>>(&self, _: &feos_core::StateHD<D>) -> [D; 2] {
        unreachable!()
    }
}
