use crate::association::{AssociationParameters, AssociationProperties};
...
impl AssociationProperties for GcPcSaftEosParameters {
    fn hs_diameter<D: num_dual::DualNum<f64>>(&self, temperature: D) -> Array1<D> {
    ...
    fn weighted_densities<D: num_dual::DualNum<f64>>(
        &self,
        state: &feos_core::StateHD<D>,
    ) -> [D; 2] {
        // temperature dependent segment diameter
        let diameter = self.hs_diameter(state.temperature);

        // Packing fractions
        let n2 = self.zeta(&diameter, &state.partial_density, [2])[0] * 6.0;
        let n3 = self.zeta(&diameter, &state.partial_density, [3])[0];
