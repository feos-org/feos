...
mod joback;
pub use joback::{Joback, JobackBinaryRecord, JobackRecord};
...
impl DipprRecord {
    pub fn new100(a: f64, b: f64, c: f64, d: f64, e: f64, f: f64, g: f64) -> Self {
        Self::DIPPR100([a, b, c, d, e, f, g])
    }

    pub fn new107(a: f64, b: f64, c: f64, d: f64, e: f64) -> Self {
        Self::DIPPR107([a, b, c, d, e])
    }

    pub fn new127(a: f64, b: f64, c: f64, d: f64, e: f64, f: f64, g: f64) -> Self {
...
mod tests {
    ...
    fn eq100() -> EosResult<()> {
        let record = PureRecord::new(
            Identifier::default(),
            0.0,
            DipprRecord::new100(276370., -2090.1, 8.125, -0.014116, 0.0000093701, 0.0, 0.0),
        );
    ...
    fn eq107() -> EosResult<()> {
        let record = PureRecord::new(
            Identifier::default(),
            0.0,
            DipprRecord::new107(33363., 26790., 2610.5, 8896., 1169.),
        );
        let dippr = Arc::new(Dippr::new_pure(record.clone())?);
        let eos = Arc::new(EquationOfState::ideal_gas(dippr.clone()));
        let temperature = 300.0 * KELVIN;
